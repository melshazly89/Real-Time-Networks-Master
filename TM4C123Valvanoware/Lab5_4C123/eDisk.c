...
// **write this function** 	
	uint32_t ROMAddress;
	uint8_t *ROMAddressptr;
	uint16_t i;
	
	ROMAddress=EDISK_ADDR_MIN+512*sector;         //The Middle of The Flash Memory
	ROMAddressptr=(uint8_t *)ROMAddress;

	for(i=0;i<512;i++)
	{
		while(ROMAddress > EDISK_ADDR_MAX){return RES_PARERR;}
		*buff=*ROMAddressptr;
		ROMAddressptr++;
		buff++;
...
// **write this function**
	uint32_t Address;
	Address=EDISK_ADDR_MIN+512*sector;         //The Middle of The Flash Memory
	while(Address > EDISK_ADDR_MAX)
	{return RES_PARERR;}
	for(int i=1;i<512;i++)
	{
		Flash_Write(Address,*(uint32_t*)buff);
		Address++;
		buff++;
	}
	return RES_OK;
...
// **write this function**
  uint32_t Address;
	Address=EDISK_ADDR_MIN;         //The Middle of The Flash Memory
	while(Address<=EDISK_ADDR_MAX)
	{
		if (Flash_Erase(Address)==ERROR) {return RES_ERROR;}  //Delete 1KB Block from Flash
		Address=Address+1024;     //Incerement of 1KB Blocks
