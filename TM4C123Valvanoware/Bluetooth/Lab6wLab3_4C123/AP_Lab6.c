...
*msg=0x82;          msg++;   // SNP Add Characteristic Value Declaration
*msg=permission;    msg++;   // 0=none,1=read,2=write, 3=Read+write, GATT Permission
*msg=properties;    msg++;   // 2=read,8=write,0x0A=read+write,0x10=notify, GATT Properties
*msg=0x00;          msg++;   // 2=read,8=write,0x0A=read+write,0x10=notify, GATT Properties
*msg=0x00;          msg++;   // RFU
*msg=0x00;          msg++;   // Maximum length of the attribute value=512
*msg=0x02;          msg++;   // Maximum length of the attribute value=512
*msg=0xFF&uuid;     msg++;   //UUID
*msg=uuid>>8;       msg++;   //UUID
SetFCS(pt);
...
while(i<20 && name[i])
{
	msg[11+i]=name[i];              // Initial user description string
	 i++;
}
msg[0]=SOF;         //Start of Frame
msg[11+i]=0;        i++;
msg[1]=i+6;         //length determined at run time 6+string length
msg[2]=0x00;        //length determined at run time 6+string length
...
msg[8]=0x00;        // Maximum Possible length of the user description string
msg[9]=i;           // Initial length of the user description string
msg[10]=0x00;       // Initial length of the user description string
...
uint8_t *pt;
	...
	int j=0;
		while(j<24 && j<stringlength)
	{
		msg[8+j]=name[j];
		j++;
	}
	msg[0]=SOF;           //SOF
	msg[8+j] = 0;         // add null termination
  msg[1] = j+3;         // frame length
	msg[2]=0x00;          // frame length
...
//****You implement this function as part of Lab 6*****
  ...
  *msg=SOF;      msg++;   //SOF
	...
	*msg=0x02;     msg++;   // GAP_ADTYPE_FLAGS,DISCOVERABLE | no BREDR
	*msg=0x01;     msg++;   // GAP_ADTYPE_FLAGS,DISCOVERABLE | no BREDR
	*msg=0x06;     msg++;   // GAP_ADTYPE_FLAGS,DISCOVERABLE | no BREDR
	*msg=0x0D;     msg++;   // Texas Instruments Company ID
	*msg=0x00;     msg++;   // Texas Instruments Company ID
	*msg=0x03;     msg++;   //TI_ST_DEVICE_ID
...
//****You implement this function as part of Lab 6*****
  ...
  while(i<24 && name[i])
	...
	msg[i]=0x05;     // length of this data
	msg[i+1]=0x12;   // GAP_ADTYPE_SLAVE_CONN_INTERVAL_RANGE
	msg[i+2]=0x50;   // DEFAULT_DESIRED_MIN_CONN_INTERVAL
	msg[i+3]=0x00;   // DEFAULT_DESIRED_MIN_CONN_INTERVAL
	msg[i+4]=0x20;   // DEFAULT_DESIRED_MAX_CONN_INTERVAL
	msg[i+5]=0x03;   // DEFAULT_DESIRED_MAX_CONN_INTERVAL
	msg[i+6]=0x02;   // length of this data
	msg[i+7]=0x0A;   // GAP_ADTYPE_POWER_LEVEL
	SetFCS(pt);     //FCS
...
uint8_t *pt;
  ...
  *msg=SOF;             msg++;    //SOF
	...
	*msg=0x00;            msg++;    // Advertise infinitely.
	*msg=0xFF&interval;   msg++;    // Advertising Interval (100 * 0.625 ms=62.5ms)
	*msg=interval>>8;     msg++;    // Advertising Interval (100 * 0.625 ms=62.5ms)
	*msg=0x00;            msg++;    // Filter Policy RFU
