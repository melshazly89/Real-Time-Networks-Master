...
void OS_Init(void){
  ...
  BSP_Clock_InitFastest();// set processor clock to fastest speed
	BSP_PeriodicTask_InitB(&runperiodicevents,1000,1);
...
// **similar to Lab 3. initialize priority field****
 ...
 status = StartCritical();
		...
		tcbs[3].next=&tcbs[4];   //Thread 3 points to 4
		tcbs[4].next=&tcbs[5];   //Thread 4 points to 5
		tcbs[5].next=&tcbs[6];   //Thread 5 points to 6
		tcbs[6].next=&tcbs[7];   //Thread 6 points to 7
		tcbs[7].next=&tcbs[0];   //Thread 7 points to 0
										
		*(tcbs[0].Blocked)=0;       //Initialize Blocking Status
		*(tcbs[1].Blocked)=0;       //Initialize Blocking Status
		*(tcbs[2].Blocked)=0;       //Initialize Blocking Status
		*(tcbs[3].Blocked)=0;       //Initialize Blocking Status
		*(tcbs[4].Blocked)=0;       //Initialize Blocking Status
		*(tcbs[5].Blocked)=0;       //Initialize Blocking Status
		*(tcbs[6].Blocked)=0;       //Initialize Blocking Status
		*(tcbs[7].Blocked)=0;       //Initialize Blocking Status
		
										
		tcbs[0].sleep=0;       //Initialize Sleeping
		tcbs[1].sleep=0;       //Initialize Sleeping
		tcbs[2].sleep=0;       //Initialize Sleeping
		tcbs[3].sleep=0;       //Initialize Sleeping
		tcbs[4].sleep=0;       //Initialize Sleeping
		tcbs[5].sleep=0;       //Initialize Sleeping
		tcbs[6].sleep=0;       //Initialize Sleeping
		tcbs[7].sleep=0;       //Initialize Sleeping
		
		tcbs[0].period=p0;       //Initialize Period
		tcbs[1].period=p1;       //Initialize Period
		tcbs[2].period=p2;       //Initialize Period
		tcbs[3].period=p3;       //Initialize Period
		tcbs[4].period=p4;       //Initialize Period
		tcbs[5].period=p5;       //Initialize Period
		tcbs[6].period=p6;       //Initialize Period
		tcbs[7].period=p7;       //Initialize Period
...
{
pt=pt->next;
if((pt->period<max)&&(pt->Blocked==0)&&(pt->sleep==0))   //Check for The Highest priority
{
	max=pt->period;      
	HPriority=pt;
}
}while(RunPt!=pt);                                        //Check for The Highest priority                             
RunPt=HPriority;
}
...
// ****IMPLEMENT THIS****
// Same as Lab 3
 *semaPt=value;    //initialize Seamphore
}
...
// Same as Lab 3
  DisableInterrupts();
	*semaPt=*semaPt-1;
	if(*semaPt<0)
	{
		RunPt->Blocked=semaPt;  //Blocked due to un availablity of resource
	...
	if(*semaPt<=0)
	  {
	    ptr=RunPt->next;
	      while(ptr->Blocked!=semaPt)
	        {
	          ptr=ptr->next;        //Seraching for Blocked Threads
	        }
					ptr->Blocked=0;            //Wakeup This one
	  }
...
// Same as Lab 3
	...
	{
		Fifo[PutI]=data;         //Put data in FIFO Buffer
		PutI=(PutI+1)%FSIZE;     //Circular array
...
// Same as Lab 3
	OS_Wait(&CurrentSize);   
	data=Fifo[GetI];        //Get Data from FIFO Buffer
	GetI=(GetI+1)%FSIZE;    //Circular Buffer
  return data;
 return data;
}
// *****periodic events****************
...
//***IMPLEMENT THIS***
  ...
  GPIO_PORTD_AMSEL_R&=~0x40;      //disable analog on PD6
  GPIO_PORTD_DEN_R|=0x40;         //configure PD6 as GPIO
	GPIO_PORTD_DIR_R&=~0x40;        //configure PD6 as Input
  GPIO_PORTD_AFSEL_R&=~0x40;      //disable alt funct on PD6
	GPIO_PORTD_PCTL_R=0x00000000;   //disable alt funct on PD6
	...
	NVIC_PRI0_R = (NVIC_PRI0_R&0x1FFFFFFF)| (priority << 29); // (g) priority on Port D edge trigger is NVIC_PRI0_R	31 – 29
